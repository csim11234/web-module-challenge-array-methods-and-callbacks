...
const finals14 = fifaData.filter((element) => {
...
console.log('taks 1b', finals14[0]['Away Team Name']);
//(c) Home Team goals for 2014 world cup final
console.log('task 1c', finals14[0]['Home Team Goals']);
//(d) Away Team goals for 2014 world cup final
console.log('taks 1d', finals14[0]['Away Team Goals']);
...
function getFinals(arr) {
   const stageArr = arr.filter((data) =>{
...
function getYears(arr, cb) {
   return cb(arr).map((data) => data.Year);
}
...
function getWinners(arr, cb) {
    return cb(arr).map((data) => data['Home Team Goals'] > data['Away Team Goals'] ? 
...
function getWinnersByYear(arr, getFinalsCB, yearsCB, winnersCB) {
   const winners = winnersCB(arr, getFinalsCB);
   const years = yearsCB(arr, getFinalsCB);
...
function getAverageGoals(data) {
    const averGoals = data.reduce(function(acc, item){
        return acc + item['Home Team Goals'] + item['Away Team Goals'];

    }, 0);
    return (averGoals / data.length).toFixed(2);
 }
